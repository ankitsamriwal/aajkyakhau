...
function renderRecipe(p){
  ...
  else if(p.src==='plan')r=RECIPES[p.i];
...
function customRecipe(src,i){return src==='ai'?(COOK&&COOK.result&&COOK.result.recipes[i]):src==='plan'?RECIPES[i]:(matchRecipes(MANUAL?MANUAL.picked:[])[i]||RECIPES[i])}
...
function sheetCustomize(){const e=S.week.slots[SHEET.ds]||{};const i=e[SHEET.sl];sheetClose();go('recipe',{src:'plan',i})}
...
const DIRECT_KITCHEN=(new URLSearchParams(location.search)).get('kitchen')==='1'||localStorage.getItem('akk_kitchen_mode')==='1';
if(DIRECT_KITCHEN){if(!S||!S.prefs){const p=defaultPrefs();p.done=true;S={prefs:p,cook:{},roll:{seen:[]}};localStorage.setItem(LS,JSON.stringify(S))}SCREEN='kitchen'}
render();
